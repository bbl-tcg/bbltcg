...
packsRouter.post("/open", async (req, res) => {
  ...
  if (user.packPoints < 1) return res.status(400).json({ error: "Not enough Pack Points." });

  const cardIds = openPack();
  await db.addToCollection(user.id, cardIds);
  await db.addPackPoints(user.id, -1);
...
packsRouter.post("/heartbeat", async (req, res) => {
  ...
  const chunks = Math.floor(elapsedMs / FIVE_MIN_MS);
  ...
  if (chunks > 0) {
    ...
    req.session.lastAccrualAt = last + chunks * FIVE_MIN_MS;
  } else {
    req.session.lastAccrualAt = last;
  }
