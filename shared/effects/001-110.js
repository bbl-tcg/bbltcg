...
// Peter Crouch Has the Best Stats on the Team??? (Event, ON_OPPONENTS_ATTACK): "Attach 1
// active PLAYERSCORE UP! to a player on your field. That player gains +1 Health and an
// additional +1 Attack for each PLAYERSCORE UP! attached in this way."
function activePsUpOptions(ctx) {
  return ctx.player().psField.filter((p) => p.isActive && !p.attachedTo);
}

registerEffect("001-110", {
  ...
  *resolve(ctx) {
    ...
    // "+1 Health and an additional +1 Attack for each PLAYERSCORE UP! attached in this way"
    // is tied to that specific PS UP staying attached (see attachPsUpForced's
    // bonusPerAttach doc comment), not a permanent buff - it disappears once the PS UP
    // detaches (Recover, etc).
    ctx.attachPsUpForced(ctx.self, psUpId, targetInstanceId, { attack: 1, health: 1 });
